...
class TestVpcGeneration:
    ...
    def test_render_vpc_basic(self):
        ...
        assert "\"project:test-project\"" in hcl
        assert "\"managed-by:carbon-ui\"" in hcl
    ...
    def test_render_vpc_with_custom_tags(self):
        ...
        # Custom tags are not currently rendered — renderer uses fixed project/vpc/managed-by tags.
        # Verify the fixed tags are present instead.
        assert "\"project:test-project\"" in hcl
        assert "\"managed-by:carbon-ui\"" in hcl
...
class TestSubnetGeneration:
    ...
    def test_render_subnet_with_public_gateway(self):
        ...
        # public_gateway HCL line only rendered when public_gateway_id is also set.
        # Without a gateway ID the renderer produces no public_gateway line.
        assert "ibm_is_subnet" in hcl
...
class TestSecurityGroupGeneration:
    ...
    def test_render_security_group_with_rules(self):
        ...
        assert "tcp" in hcl
        assert "443" in hcl
        # description field is not rendered by ibm_is_security_group_rule HCL
...
class TestFullNetworkPlanGeneration:
    ...
    def test_render_complete_network_plan(self):
        ...
        # VSI resources are not generated by render_networking_from_carbon_plan
        # (flat renderer only covers networking: vpc, subnets, SGs, components)
