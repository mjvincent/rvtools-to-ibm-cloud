...
20. Use the remediation queue to route to blockers or apply reviewed high-confidence suggestions.
21. Use `Preview Terraform` to inspect generated package files in the UI.
22. Read each Export status or error message for the action result and next recommended step.
23. Download the readiness report if a review artifact is needed.
24. Click `Download Terraform ZIP` after preflight blockers are resolved.
