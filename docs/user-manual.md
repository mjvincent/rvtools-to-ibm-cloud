...
- **Export readiness guidance**: Carbon Export includes a readiness checklist, summary metrics, current-state banner, recommended resolution order, backend package preflight, blocker routing, Terraform package preview, handoff guidance, and ZIP download gating. The summary metrics show saved-project state, planning gaps, preflight blockers/warnings, remediation queue count, preview status, and ZIP readiness. The current-state banner keeps the active Export stage visible and offers a next-action control when the app can route directly. The recommended resolution order points users to the next export action from saved project, planning gaps, preflight, blockers, preview, and handoff download. Export action feedback explains what happened and what to do next after preflight, preview, planning-state, readiness-report, and ZIP actions. Package preflight findings can be searched by VM, category, message, or fix context and filtered by severity, with active filter indicators and a clear action, before being grouped by severity and category, with blockers before warnings, so reviewers can route the highest-risk items first. When more than five findings exist, Carbon starts with the top findings and lets users expand or collapse the full list. The handoff guidance explains key package controls, generated files, owners, and the boundary that Terraform execution remains outside the app. ZIP download runs preflight first and stops when blockers remain.
...
- **Completion checklists**: Each main Carbon workflow shows a compact, collapsible `Complete when` checklist below the header so users can see finish criteria without opening help, then collapse it when they need more workspace.
...
3. Use each workflow's `Complete when` checklist to understand what done means before moving forward.
4. Use workflow-header `Step help` or shell-level `Help` when the step purpose or completion criteria are unclear.
5. Use `Open user guide` if you want a separate instruction window open while working.
...
16. Use the current-state banner to confirm the active Export stage and trigger the next available action.
17. Follow the recommended resolution order to see the next export action before jumping between panels.
18. Review the handoff guidance to understand package controls, key generated files, owners, and operator responsibilities.
19. Save or load the desired project, then run `Run preflight`.
20. Search or filter package preflight findings when needed, use active filter indicators or `Clear filters` to manage the filtered view, then review grouped findings by severity and category, starting with blockers; expand the full list when additional findings are hidden.
21. Use the remediation queue to route to blockers or apply reviewed high-confidence suggestions.
22. Use `Preview Terraform` to inspect generated package files in the UI.
23. Read each Export status or error message for the action result and next recommended step.
24. Download the readiness report if a review artifact is needed.
25. Click `Download Terraform ZIP` after preflight blockers are resolved.
...
| Export readiness and Terraform ZIP | Implemented | Includes package parity inventory, summary metrics, handoff guidance, preview, preflight, queue routing, ZIP download, and readiness report. |
