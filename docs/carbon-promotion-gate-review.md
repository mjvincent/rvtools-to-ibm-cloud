...
| Gate 3: Network Planning | Partial | Carbon supports VPC/subnet/security/storage/wave/network component planning, saved network plans, diagram display, Terraform generation, package inventory parity status, and backend package preflight feedback in Export. | Add richer network component editing, clickable/editable diagram nodes, richer validation beyond package preflight, and Terraform preview. | Phase 5 and Phase 6 |
...
- Extend Carbon-side preflight feedback with fix-routing and quick actions.
